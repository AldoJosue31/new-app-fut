-- Periods remain explicitly allowed in every name pattern.
...
alter table public.players add constraint players_first_name_valid
  check (char_length(btrim(first_name)) between 1 and 25
...
alter table public.players add constraint players_last_name_valid
  check (char_length(btrim(last_name)) between 1 and 35
...
alter table public.teams add constraint teams_name_valid
  check (char_length(btrim(name)) between 1 and 35
...
alter table public.teams add constraint teams_delegate_name_valid
  check (delegate_name is null or (char_length(btrim(delegate_name)) between 1 and 45
