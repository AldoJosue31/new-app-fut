...
// Importamos TabsNavigation directamente
import { Card, CardHeader, BtnNormal, Btnsave, InputNumber, InputText2, Modal, TabsNavigation } from "../../../../index";
...
export function TorneoDefinicionTab({ 
    ...
    isLoading // <=== CAMBIO 2: AGREGAR ESTA PROP
}) {
  ...
  const handleSaveConfig = () => {
    // Guardamos el objeto 'form' actual en el almacenamiento del navegador
    localStorage.setItem("torneo_reglas_draft", JSON.stringify(form));
    
    // Cerramos el modal
    setShowConfigModal(false);
    
    // Opcional: Feedback visual simple
    // alert("Configuración guardada temporalmente"); 
  };
...
<TeamsList>
        {/* === CAMBIO 3: LÓGICA DE CARGA === */}
        {isLoading ? (
            // SI ESTÁ CARGANDO: Muestra 3 o 4 Skeletons
            <>
                <TableRowSkeleton />
                <TableRowSkeleton />
                <TableRowSkeleton />
            ...
            // SI YA CARGÓ: Muestra la lista real o el mensaje de vacío
        ...
        {/* === FIN DEL CAMBIO === */}
            ...
            <div style={{ marginTop: '20px', borderTop: `1px solid ${v.bg4}`, paddingTop:'20px', display:'flex', justifyContent:'end' }}>
                <Btnsave titulo={loading ? "Creando..." : "Iniciar Torneo"} bgcolor={v.colorPrincipal} icono={<v.iconoguardar />} funcion={onSubmit} disabled={loading || !divisionName || participatingTeams.length < 2 || !form.season} />
            </div>
        </Card>
        ...
        <Modal isOpen={showConfigModal} onClose={() => setShowConfigModal(false)} title="Configurar Reglas" width="650px">
            <ModalContentStyled>
                ...
                {/* --- USAMOS EL COMPONENTE REUTILIZABLE AHORA --- */}
                <TabsNavigation 
                    tabs={configTabList} 
                    activeTab={configTab} 
...
<div className="modal-actions">
                    <Btnsave 
                        ...
                        // CAMBIO: Usamos la nueva función en lugar de solo cerrar
...
const TeamsList = styled.div` background: ${({theme})=>theme.bgtotal}; border-radius: 10px; padding: 10px; max-height: 250px; overflow-y: auto; border: 1px solid ${({theme})=>theme.bg4}; margin-bottom: 15px; &::-webkit-scrollbar { width: 6px; } &::-webkit-scrollbar-thumb { background: ${v.bg4}; border-radius: 3px; } `;
...
// Estilo para el contenido de los tabs (animación de entrada)
