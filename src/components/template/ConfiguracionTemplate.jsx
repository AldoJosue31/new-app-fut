...
import { 
  Btnsave, InputText2, BtnNormal, ContentContainer, Card, CardHeader, Badge
} from "../../index";
...
export function ConfiguracionTemplate({ state, setState }) {
  ...
  useEffect(() => { if (profile?.full_name) { setNombre(profile.full_name); } }, [profile]);

  useEffect(() => {
    const fetchLeagueName = async () => {
      if (!user) return;
      try {
        const { data, error } = await supabase.from('leagues').select('name').eq('owner_id', user.id).single();
        if (data && !error) { setLeagueName(data.name); }
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault(); setLoadingUpdate(true);
    try {
      const { error } = await supabase.from('profiles').update({ full_name: nombre }).eq('id', user.id);
      if (error) throw error;
      await fetchProfile(user.id); alert("Perfil actualizado");
    } catch (err) { alert("Error: " + err.message); } finally { setLoadingUpdate(false); }
  };

  const handleLinkGoogle = async () => {
    setLoadingLink(true);
    try {
      const { error } = await supabase.auth.linkIdentity({ provider: 'google', options: { redirectTo: `${window.location.origin}/configuracion`, } });
      if (error) throw error;
    } catch (err) { alert("Error: " + err.message); setLoadingLink(false); }
  };
  ...
  const handleUnlinkGoogle = async () => {
    ...
    try {
      ...
      if (session?.user) { useAuthStore.setState({ user: session.user }); } 
      else { const { data: { user: updatedUser } } = await supabase.auth.getUser(); useAuthStore.setState({ user: updatedUser }); }
      alert("Desvinculado");
    } catch (err) { alert("Error: " + err.message); } finally { setLoadingUnlink(false); }
  };

  const getRoleLabel = (role) => {
    switch(role) { case 'admin': return 'Administrador'; case 'manager': return 'Gestor'; case 'user': return 'Jugador'; default: return 'Sin rol'; }
  };
...
const LinkedAccounts = styled.div` h4 { margin: 0 0 20px 0; font-size: 16px; } .account-item { display: flex; align-items: center; justify-content: space-between; padding: 15px; border-radius: 12px; background: ${({ theme }) => theme.bgtotal}; border: 1px solid ${({ theme }) => theme.bg4}; gap: 10px; flex-wrap: wrap; } .left { display: flex; align-items: center; gap: 12px; } .google-icon { font-size: 28px; display: flex; } .google-avatar { width: 38px; height: 38px; border-radius: 50%; object-fit: cover; border: 2px solid ${({ theme }) => theme.bg4}; } .info { display: flex; flex-direction: column; span { font-weight: 600; font-size: 15px; max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } small { opacity: 0.6; font-size: 13px; } } .right { display: flex; align-items: center; } .actions { display: flex; justify-content: flex-end; margin-top: 20px; } `;
const ActionsGroup = styled.div` display: flex; align-items: center; gap: 10px; `;
const StatusBadge = styled.span` font-size: 12px; font-weight: 700; color: #2ed573; padding: 6px 12px; background: rgba(46, 213, 115, 0.1); border-radius: 20px; border: 1px solid rgba(46, 213, 115, 0.2); white-space: nowrap; `;