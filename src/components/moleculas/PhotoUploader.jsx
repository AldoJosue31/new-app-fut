...
// Memo evita que el componente se re-procese si sus props no cambian de valor
export const PhotoUploader = memo(function PhotoUploader({ 
  ...
  // OPTIMIZACIÓN: Solo creamos la instancia de Image una vez por ciclo de vida
  ...
  const handleManualAdjust = async (e) => {
    ...
    if (originalUrl) {
      try {
        ...
        showToast("Error cargando la imagen para editar", "error");
      } finally {
  ...
  const handleMouseDown = (e) => { 
    setIsDragging(true); 
    setStartPan({ x: e.clientX - crop.x, y: e.clientY - crop.y }); 
  };
  
  const handleMouseMove = (e) => { 
    if (!isDragging) return; 
    setCrop({ x: e.clientX - startPan.x, y: e.clientY - startPan.y }); 
  };
  ...
  useEffect(() => {
    if (isCropping && tempImgSrc && canvasRef.current) {
        ...
        img.onload = () => {
             ...
             const scaledW = img.width * currentScale;
             const scaledH = img.height * currentScale;
             ctx.drawImage(img, (size - scaledW)/2 + crop.x, (size - scaledH)/2 + crop.y, scaledW, scaledH);
        };
    }
...
const CropModalOverlay = styled.div`
  ...
  .crop-card {
    background: ${({theme}) => theme.bgcards}; padding: 20px; border-radius: 20px; width: 90%; max-width: 420px;
