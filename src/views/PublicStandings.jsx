...
export const PublicStandings = ({
  ...
  const [torneo, setTorneo] = useState(
    initialBundle?.tournament || null,
  );
  const [partidos, setPartidos] = useState(
    Array.isArray(initialBundle?.matches) ? initialBundle.matches : [],
  );
  const [equipos, setEquipos] = useState(
    Array.isArray(initialBundle?.teams) ? initialBundle.teams : [],
  );
  const [jornadas, setJornadas] = useState(
    Array.isArray(initialBundle?.jornadas) ? initialBundle.jornadas : [],
  );
  const [goleadores, setGoleadores] = useState(
    Array.isArray(initialBundle?.scorers) ? initialBundle.scorers : [],
  );
  const [goalEvents, setGoalEvents] = useState(
    Array.isArray(initialBundle?.goal_events)
      ? initialBundle.goal_events
      : [],
  );
  ...
  useEffect(() => {
    if (initialBundle || initialError) return undefined;
    ...
    const fetchData = async () => {
      try {
        ...
        if (!ignore) setError("ERROR_GENERIC");
      } finally {
  ...
  }, [initialBundle, initialError, torneoId]);
