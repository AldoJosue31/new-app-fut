...
export const createHandler = (dependencies = {}) => {
  ...
  const authorizeManager = dependencies.requireManager;
  ...
  return async function handler(req, res) {
    ...
    try {
      await authorizeManager(req);
      const { client, user } = await authorizeUser(req);
      ...
      if (deleteError) {
        console.error("Delegate account deletion failed.", {
          code: deleteError.code || null,
          name: deleteError.name || "Error",
